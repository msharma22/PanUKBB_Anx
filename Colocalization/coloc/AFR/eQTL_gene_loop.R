...
for (population in popList) {
  ...
  geneFile_path <- "/home/maya/COLOC/eQTL_data/Amygdala/AFR_Anx_colocs/genes.csv"
  geneFile <- fread(geneFile_path)
  data <- fread('/home/maya/prediXcan/prediXcanOUTPUT/ANX/AFR/AFR_Anx_' %&% tissue %&% '.csv')
...
eQTL_Bigdir = "/home/grace/coloc/eQTL_data/" %&% tissue %&% "/" %&% "Brain_CBG_eQTL.csv" 
eQTLs <- fread(eQTL_Bigdir) #read in eQTLs from tissue 
...
for (population in popList) {
  ...
  geneFile <- fread(geneFile_path)
  data <- fread('/home/maya/panUKBB-prediXcan/prediXcanOUTPUT/ANX/AFR/AFR_Anx_Cerebellar_Hemisphere.csv')
  if ("gene_name" %in% colnames(geneFile)){
    join <- right_join(data,geneFile, by = "gene_name")
    gene_ids <- as.list(join$gene)
    genes <- as.list(join$gene_name)
